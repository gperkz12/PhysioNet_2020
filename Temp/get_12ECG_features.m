...
save('tmpdata','data');
	...
	try
                ...
                features(25) = size(data,2);
                
                % Spectral Features - BMW
                fft_leads = zeros(num_leads,2^10);
                for i = 1:num_leads
                    tmpfft = fft(Lead12wGain(i,:),2^15);
                    % Keep low frequency data
                    fft_leads(i,:) = abs(tmpfft(1:2^10));
                end
                fft_average = mean(fft_leads);
                
                mu = mean(fft_average);
                sigma = std(fft_average);
                kurt = kurtosis(fft_average);
                skew = skewness(fft_average);
                [~,max_loc] = find(fft_average==max(fft_average));
                max_loc = max_loc(1);
                
                features(26:30) = [mu, sigma, kurt, skew, max_loc];
